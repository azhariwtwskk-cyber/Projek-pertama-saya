  ...
  Future<void> _submit(PmTask task) async {
    if (!task.allMandatoryChecked) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('Please complete all mandatory checklist items first.'),
    ...
    try {
      await ref
          .read(pmActionsControllerProvider)
          .complete(task.id, evidencePhotos: _evidencePhotos);
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('PM task completed.')));
      ...
      body: taskAsync.when(
        ...
        error: (e, _) => Center(
            child: AppStateView.error(
                onRetry: () =>
                    ref.invalidate(pmTaskDetailProvider(widget.taskId)))),
      ...
      children: [
        ListView(
          ...
          children: [
            ...
            // Preventive Maintenance schedules have no checklist on the
            // real backend (`cpms_pm_schedules` has no checklist columns
            // — see mobile/docs/INTEGRATION_REPAIR_REPORT.md). A real,
            // working, category-specific checklist exists for Asset
            // Inspection instead (QR scan flow). Only show this card if
            // a checklist was actually supplied, so PM tasks don't
            // render an empty "Checklist" section for nothing to tick.
