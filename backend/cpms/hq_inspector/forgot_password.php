...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$errors && 'hq_inspector'==='property_portal' && $propertyId<1) $errors[]='Kod property tidak sah.';
    if (!$errors) {
        $stmt=$conn->prepare("SELECT id,full_name,email FROM hq_inspectors WHERE (username=? OR email=?) AND status='active' AND 1=1 LIMIT 1");
        if ($stmt) { $stmt->bind_param('ss', $login, $login); $stmt->execute(); $u=$stmt->get_result()->fetch_assoc(); $stmt->close();
            if ($u) { $token=cpmsPasswordResetCreate($conn,'hq_inspector',(int)$u['id'],(string)$u['email']);
                if ($token) { $url=cpmsPasswordResetBaseUrl() . '/cpms/hq_inspector/reset_password.php?token=' . urlencode($token); cpmsPasswordResetSend((string)$u['email'],(string)$u['full_name'],$url); }
            }
        }
        $sent=true;
    }
