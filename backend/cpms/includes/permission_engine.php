...
function cpmsPermissionGrantRevision(
    mysqli $db,
    int $userId,
    ?int $propertyId
): string {
    if ($userId <= 0 || $propertyId === null) {
        return 'none';
    }
    ...
    try {
        $stmt = $db->prepare(
            "SELECT COUNT(*) AS total,
                    COALESCE(
                        GROUP_CONCAT(
                            permission_id ORDER BY permission_id SEPARATOR ','
                        ),
                        'none'
                    ) AS revision
    ...
    /*
     * Keep older installations operational while Migration 0054 is
     * waiting to be run. The role permission engine remains available.
     */
    if (!$stmt) {
        return 'table-unavailable';
    }
    ...
    try {
        $grantRevisionExecuted = $stmt->execute();
    } catch (Throwable $exception) {
        $grantRevisionExecuted = false;
    }
    if (!$grantRevisionExecuted) {
        $stmt->close();
...
function cpmsPermissionLoad(mysqli $db, bool $refresh = false): array
{
    ...
    $grantRevision = cpmsPermissionGrantRevision(
        $db,
        $userId,
        $propertyId
    );
    ...
    if ($propertyId !== null) {
        ...
        if ($grantStmt) {
            $grantStmt->bind_param('ii', $userId, $propertyId);
            try {
                $grantExecuted = $grantStmt->execute();
            } catch (Throwable $exception) {
                $grantExecuted = false;
            }
            if ($grantExecuted) {
                $grantResult = $grantStmt->get_result();
                while ($grantRow = $grantResult->fetch_assoc()) {
                    $code = (string) (
                        $grantRow['permission_code'] ?? ''
                    );
                    if ($code !== '') {
                        $permissions[$code] = true;
                    }
