...
function cpmsUnifiedAuthSelectContext(
    ...
    foreach ($roles as $assignment) {
        ...
        if (
            $role === 'resident'
            && defined('CPMS_ALLOW_RESIDENT_AUTH')
            && CPMS_ALLOW_RESIDENT_AUTH === true
        ) {
            return [
                'role' => $role,
                'property_id' => (int) $propertyId,
...
): array
{
    ...
    if (
        $role === 'resident'
        && $sourceTable === 'cpms_residents'
        && $sourceId > 0
    ) {
        $session['resident_id'] = $sourceId;
        $session['resident_name'] = $fullName;
        $session['resident_role'] = 'resident';
        $session['resident_property_id'] = $propertyId;
        $session['resident_last_activity'] = time();
